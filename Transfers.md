...
Here we have a write transfer where we write data from master to slave, so HWRITE=1. HREADY=1 always. Data is sent in the data phase.   
  ...
  3. After the slave has sampled the address and control it cam start to drive the appropiate HREADYOUT response. this response is sampled by the manager on the third risig edge of HCLK. 
